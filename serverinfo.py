...
async def server(ctx, param: str):
    if param == 'info':
        ...
        try:
            requests.post('https://panel.sillydev.co.uk/api/client/servers/54b39149/power', json = json, headers=headers)
        ...
        try:
            print(requests.post('https://panel.sillydev.co.uk/api/client/servers/54b39149/power', json = json, headers=headers))
            await asyncio.sleep(5)
            embed = discord.Embed(
